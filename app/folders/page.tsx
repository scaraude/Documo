...
export default function FoldersPage() {
    ...
    const FolderGridItem = ({ folderType }: {
        ...
        return (
            <div className="group cursor-pointer" onClick={() => router.push(ROUTES.FOLDER_TYPES.DETAIL(folderType.id))}>
                ...
                <div className="relative">
                    <div className="absolute bottom-4 right-4 z-50 group/button">
                        ...
                        >
                            <div className="relative">
                                {/* Tooltip */}
                                <div className={`absolute bottom-full right-0 mb-2 px-3 py-1 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover/button:opacity-100 transform scale-95 group-hover/button:scale-100 transition-all pointer-events-none whitespace-nowrap z-60 ${groupTransition}`}>
                                    Créer un dossier
                                    <div className="absolute top-full right-4 w-2 h-2 bg-gray-900 transform rotate-45 -mt-1"></div>
                                </div>
                                <button className={`w-10 h-10 bg-blue-600 text-white rounded-full hover:bg-blue-700 hover:scale-110 transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl ${groupTransition}`}>
                    ...
                    {/* Folder Tab */}
                    <div className={`absolute -top-3.5 left-0 bg-white w-24 h-7 rounded-t-lg rounded-tl-sm border-2 border-gray-200 border-b-0 group-hover:bg-gray-50 group-hover:border-blue-300 ${groupTransition}`}></div>

                    {/* Folder Body */}
                    <div className={`h-48 w-80 md:w-80 sm:w-72 bg-white border-2 border-gray-200 rounded-lg rounded-tl-none p-5 transition-all duration-300 group-hover:shadow-xl group-hover:bg-gray-50 group-hover:border-blue-300 relative overflow-hidden ${groupTransition}`}>
                        {/* Folder Content */}
