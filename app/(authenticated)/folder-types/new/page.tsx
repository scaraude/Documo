...
export default function NewFolderTypePage() {
  ...
  const handleSubmit = async (data: CreateFolderTypeParams) => {
    try {
      setError(null);
      await createFolderTypeMutation.mutateAsync(data);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 p-4 bg-[var(--documo-blue-light)] border border-[var(--documo-blue)]/20 rounded-md">
        <h3 className="text-sm font-medium text-[var(--documo-blue-deep)] mb-1">
          Qu&apos;est-ce qu&apos;un modèle de dossier ?
        </h3>
        <p className="text-sm text-[var(--documo-text-secondary)]">
          Un modèle de dossier définit quels documents sont nécessaires pour un
          processus spécifique (ex : dossier locatif, demande de vente, dossier
          d&apos;achat). Une fois créé, tu pourras ouvrir plusieurs dossiers
          basés sur ce modèle.
        </p>
      </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <FolderTypeForm
          ...
          isLoading={createFolderTypeMutation.isPending}
        />
      </div>
    </div>
  );
