...
describe('Request Repository', () => {
    ...
    describe('createRequest', () => {
        ...
        it('devrait utiliser le nombre de jours d\'expiration personnalisé', async () => {
            ...
            mockPrisma.documentRequest.create.mockResolvedValue(mockRequestWithCustomExpiry);
        ...
        it('devrait gérer les erreurs lors de la création', async () => {
            ...
            // WHEN/THEN
            await expect(createRequest(requestData)).rejects.toThrow('Failed to create request');
        });
    });
    ...
    describe('deleteRequest', () => {
        ...
        it('devrait gérer les erreurs lors de la suppression', async () => {
            // GIVEN
            mockPrisma.documentRequest.delete.mockRejectedValue(new Error('Delete error'));

            // WHEN/THEN
            await expect(deleteRequest('1')).rejects.toThrow('Failed to delete request');
        });
    });
    ...
    describe('getRequestById', () => {
        it('devrait récupérer une demande par ID', async () => {
            // GIVEN
            mockPrisma.documentRequest.findUnique.mockResolvedValue(mockPrismaRequest);
        ...
        it('devrait gérer les erreurs de recherche', async () => {
            // GIVEN
            mockPrisma.documentRequest.findUnique.mockRejectedValue(new Error('Query error'));

            // WHEN/THEN
            await expect(getRequestById('1')).rejects.toThrow('Failed to fetch request');
        });
    });
});