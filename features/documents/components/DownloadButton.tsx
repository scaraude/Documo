...
export function DownloadButton({ document }: DownloadButtonProps) {
    const classNames = 'inline-flex items-center shadow-sm px-2.5 py-0.5 border border-gray-300 text-sm leading-5 font-medium rounded-full text-gray-700 bg-white hover:bg-gray-50'
    ...
    if (error) {
        return (
            <Button className={classNames} onClick={(e: React.MouseEvent) => { e.stopPropagation(); decryptDocument() }
            }>
