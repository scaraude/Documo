...
export const folderTypesRouter = router({
    getAll: publicProcedure.query(async () => {
        try {
            ...
            logger.info({ count: result.length }, 'Folder types fetched successfully');
            return result;
        } catch (error) {
            logger.error({ error: error instanceof Error ? error.message : error }, 'Error fetching folder types');
    ...
    getById: publicProcedure.input(z.object({ id: z.string().uuid() }))
        .query(async ({ input }) => {
            try {
                ...
                logger.info({ folderTypeId: input.id, found: !!result }, 'Folder type fetch completed');
                return result;
            } catch (error) {
                logger.error({ folderTypeId: input.id, error: error instanceof Error ? error.message : error }, 'Error fetching folder type');
    ...
    create: publicProcedure.input(CreateFolderTypeSchema)
        .mutation(async ({ input }) => {
            try {
                ...
                logger.info({ folderTypeId: result.id, folderTypeName: result.name }, 'Folder type created successfully');
                return result;
            } catch (error) {
                logger.error({ folderTypeName: input.name, error: error instanceof Error ? error.message : error }, 'Error creating folder type');
    ...
    isInUsed: publicProcedure.input(z.object({ id: z.string().uuid() }))
        .query(async ({ input }) => {
            try {
                ...
                logger.info({ folderTypeId: input.id, inUse: result }, 'Folder type usage check completed');
                return result;
            } catch (error) {
                logger.error({ folderTypeId: input.id, error: error instanceof Error ? error.message : error }, 'Error checking folder type usage');
    ...
    update: publicProcedure.input(z.object({ id: z.string().uuid(), params: UpdateFolderTypeSchema }))
        .mutation(async ({ input }) => {
            try {
                logger.info({ folderTypeId: input.id, updateData: input.params }, 'Updating folder type');
                const result = await folderTypesRepository.updateFolderType(input.id, input.params);
                logger.info({ folderTypeId: input.id, folderTypeName: result.name }, 'Folder type updated successfully');
                return result;
            } catch (error) {
                logger.error({ folderTypeId: input.id, error: error instanceof Error ? error.message : error }, 'Error updating folder type');
    ...
    delete: publicProcedure.input(z.object({ id: z.string().uuid() }))
        .mutation(async ({ input }) => {
            try {
                ...
                logger.info({ folderTypeId: input.id }, 'Folder type deleted successfully');
            } catch (error) {
                logger.error({ folderTypeId: input.id, error: error instanceof Error ? error.message : error }, 'Error deleting folder type');
